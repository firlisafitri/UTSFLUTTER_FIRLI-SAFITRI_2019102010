...
class _EditState extends State<Edit> {
  ...
  Future _getData() async {
    try {
      final response = await http.get(Uri.parse(
          ...
          "http://192.168.43.72/utsflutter/Api/detail.php?id='${widget.id}'"));
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          salary = TextEditingController(text: data['salary']);
        });
      }
    } catch (e) {
      print(e);
  ...
  Future _onUpdate(context) async {
    try {
      return await http.post(
        Uri.parse("http://192.168.43.72/utsflutter/Api/update.php"),
        body: {
          "id": widget.id,
          "name": name.text,
  ...
  Future _onDelete(context) async {
    try {
      return await http.post(
        Uri.parse("http://192.168.43.72/utsflutter/Api/delete.php"),
        body: {
          "id": widget.id,
        },
      ).then((value) {
